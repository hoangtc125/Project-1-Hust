...
//    Tiem su phu thuoc
	...
	public String listuser(@RequestParam(required = false, name = "sort", defaultValue = "name") String typeSort,
			@RequestParam(required = false, name = "mssv") String mssv, @ModelAttribute("user") User user,
			Model model) {
		if (mssv != null) {
	...
	public String listuserdeleted(@RequestParam(required = false, name = "sort", defaultValue = "") String typeSort,
			@RequestParam(required = false, name = "mssv") String mssv, @ModelAttribute("user") User user,
			Model model) {
		if (mssv != null) {
			System.out.println(mssv);
	...
	public String follow(@PathVariable(required = true, name = "id") int id, Model model, HttpServletRequest request) {
		...
		Date date = new Date();
		notiService.save(new Noti(user.getUsername(), AppUtils.getLoginedUser(request.getSession()) + " followed you",
				0, "/prj1.com/", date.toString()));
	...
	public String unFollow(@PathVariable(required = false, name = "id") int id, Model model,
		...
		Date date = new Date();
		notiService.save(new Noti(user.getUsername(), AppUtils.getLoginedUser(request.getSession()) + " unfollowed you",
				0, "/prj1.com/", date.toString()));
		return "redirect:/user-view/" + id + "/" + user.getUsername();
	}
	...
	public String viewuser(HttpServletRequest request, @PathVariable(required = false, name = "id") int id,
