							...
							<c:forEach var="item" items="${listItem}">
								<tr>
									...
									<td class="text-left"><img
										src="${item.getProduct().image}" class="card-img-top"
										alt="..." style="width: 55px; height: auto;">${item.getProduct().getName()}</td>
									<td class="unit">${item.getProduct().getPrice()}VND</td>
									<td class="qty">${item.getQuan()}</td>
									<td class="total-price">${item.getProduct().getPrice() * item.getQuan()}
										VND</td>
								</tr>
							</c:forEach>
						</c:if>
